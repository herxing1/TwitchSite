(() => {
  ...
  const heading=document.createElement('p');heading.className='menu-heading';heading.textContent='ON SE RETROUVE OÙ ?';nav.prepend(heading);
  ...
  function close(focus=false){header.classList.remove('menu-open');backdrop.classList.remove('visible');button.setAttribute('aria-expanded','false');label.textContent='Menu';if(focus)button.focus();}
  function layout(){close();if(twitch){if(mobile.matches)nav.append(twitch);else header.append(twitch);}}
  button.addEventListener('click',()=>{const open=button.getAttribute('aria-expanded')!=='true';header.classList.toggle('menu-open',open);backdrop.classList.toggle('visible',open);button.setAttribute('aria-expanded',String(open));label.textContent=open?'Fermer':'Menu';});
